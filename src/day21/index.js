...
const playerWins = (playerHp, playerDamage, playerArmor, bossHp, bossDamage, bossArmor) => {
...
const equipRing = (weapon, armor, tryEquipment) => {
...
const equipArmor = (weapon, tryEquipment) => {
  equipRing(weapon, [0,0,0], tryEquipment)
  for (var i = 0; i < armor.length; i++) {
    equipRing(weapon, armor[i], tryEquipment)
  }
}

const equipPermutations = (tryEquipment) => {
  weapons.forEach(weapon => equipArmor(weapon, tryEquipment))
}
...
const part1 = (rawInput) => {
  ...
  const tryEquipment = ([cost, damage, armor]) => {
    ...
    if (playerWins(100, damage, armor, ...boss)) {
      min = Math.min(min, cost)
    }
  }

  equipPermutations(tryEquipment)
